...
const Board = () => {
  ...
  const handleAdd = (match) => {
    axios
      .put(`/match/insert/${match._id}`, {
        ...
        isCompleted: false,
        remarks: "MORE RICE",
        dishOrdered: { itemName: "Duck Rice", itemPrice: 3.0 },
      })
      .then(function (response) {
        // handle success
  ...
  if (matches[0] === "loading") return <h1>Loading ..... </h1>;
              ...
              {matches?.map((match, index) => {
                ...
                return (
                  <tr className="bg-emerald-200" key={match._id}>
                    ...
                    <td>
                      {!isConfirmedOrder && (
                        <button onClick={() => debouncedSavePlus(match)}>
                          +
                        </button>
                      )}
                      {isConfirmedOrder && (
                        <button onClick={() => debouncedSaveMinus(match)}>
                          -
                        </button>
                      )}
                    </td>
